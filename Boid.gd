...
var predator_perception_radius = 200
...
var predator_separation_force = 24.0
var avoidance_force = 12.0
...
func _ready():
	...
	modulate = Color(0,0,0)#colors[rand_range(0, colors.size())]


func _process(delta):
	
	var neighbors = get_neighbors(perception_radius, boids)
	var predators = get_neighbors(predator_perception_radius, predator_boids)
	...
	acceleration += process_predator_seperation(predators) * predator_separation_force
	...
	$AnimationPlayer.playback_speed = velocity.length_squared() * 0.0001
...
func process_predator_seperation(predators):
	var vector = Vector2()
	var close_neighbors = []
	for boid in predators:
		if position.distance_to(boid.position) < predator_perception_radius / 2:
...
func get_neighbors(view_radius, boids_list):
	var neighbors = []
